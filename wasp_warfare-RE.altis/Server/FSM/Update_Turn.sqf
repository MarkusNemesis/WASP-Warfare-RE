/* Markus -
This needs to be rewritten to a state of useability. 
Does not validate whether a player has over-ordered units
and will overflow the unit cap.
*/
				...
				if (typeName _FirstVehicle == "ARRAY") then    {
					...
					if ((time >= (_Time + _TimeCreate))) then     {
						...
						if (_VehType isKindOf "Man") then {
							[_Owner, [[_VehType, _VehRank, _Group, _RespawnPos], {_this call Create_Unit}], ObjNull] call Send_Run;
						}       else {
							[_Owner, [[_BuildingObject, _VehType, _VehRank, _Group, _RespawnPos, _Driver, _Guner, _Commander, _Lock], {_this call Create_Vehicle}], ObjNull] call Send_Run;
						};
...
if (count Server_BuyTurn > 0) then {
	{
		...
		if (count(_FactoryTurn) > 0) then {
			Public_BuyTurnNumber set [count(Public_BuyTurnNumber), [_FactoryObject, count(_FactoryTurn)]]
		}			
	} forEach Server_BuyTurn;
};
