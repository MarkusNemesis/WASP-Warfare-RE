...
_SetImage ={
	...
	_Filled  = if (_Type != "") then {true} else {false};
	...
	_Cost  = 0;
	{
		...
		for "_i" from 0 to (count (_PreSell) - 1) do  {
			...
			if (_ArrayIndex != -1) then   {
				...
				_SellList set [count(_SellList), [_GearType, - _GearCost]]; // Markus - Remove sell-back penalty
			};
		};
...
_Buy ={
	...
	_Unit   = _UnitsArray select _SelectedNum;
	_Pay   = {
		if (_Cost < 0) then {_Cost = 0}; // Markus - Disable sell-back to disallow players to profit from selling AI gear.
	...
	_Cost call _UpdateCostMoney;
	//hint format["Buy"]; Markus - Remove debug hints
};
_Close ={
