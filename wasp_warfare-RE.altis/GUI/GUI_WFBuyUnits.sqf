...
_AddFactoryBuy  ={
	...
	// Markus - Find the nearest factory of this type and select it
	private ["_nearestFac","_nearestFacID"];
	_nearestFac = 999999;
	_nearestFacID = 0;
	lbClear _FactoryChoose;
	{
		_tmpDist = round((_x select 3) distance Player);
		LbAdd [_FactoryChoose, format["%1 - %2m", _AddFName, _tmpDist]];
		if (_tmpDist < _nearestFac) then {_nearestFacID = _ForEachIndex};
	} forEach _AddBlds;
	lbSetCurSel [_FactoryChoose, _nearestFacID];
	/// Markus
	lnbClear  _BuyList;
	{
...
_BuyForAllFType ={
	...
	_Purchase = [_FType, _Building, _Vehicle select 1, _Vehicle select 2, time,  _Vehicle select 5, {}, Group Player, _SpecBool select 0, _SpecBool select 1, _SpecBool select 2, _SpecBool select 3];
...
_Buy ={
	if (BuyButtonCooldown > time) exitWith {hint parseText format ["<t align='center' color='#ff0000'>%1</t><br /><t align='center'></t>",  "You're clicking too fast<br />Please wait 1 second"];};
	...
	_Cost  = if (!isNil "_Vehicle") then {_Vehicle select 4} else {100};
	if (_Money >= _Cost) then {
		BuyButtonCooldown = time + 2; // Markus - Temp fix
		private["_UnitsInVeh"];
		if (count (units (player)) <  paramsArray select 0) then  {
			...
			hint parseText format ["<t align='center' color='#ff0000'>%1</t><br /><t align='center'></t>",  "Unit Limit Reached"];
		};
	} else {
		hint parseText format ["<t align='center' color='#ff0000'>%1</t><br /><t align='center'></t>",  "Insufficient Funds"];
	};
};
